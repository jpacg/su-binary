...
int run_daemon() {
    ...
    if (bind(fd, (struct sockaddr*)&sun, strlen(DAEMON_SOCKET_NAME) + offsetof(struct sockaddr_un, sun_path)  + 1) < 0) {
        PLOGE("daemon bind");
        goto err;
    }
...
int connect_daemon(int argc, char *argv[], int ppid) {
    ...
    if (0 != connect(socketfd, (struct sockaddr*)&sun, strlen(DAEMON_SOCKET_NAME) + offsetof(struct sockaddr_un, sun_path)  + 1)) {
        PLOGE("connect");
        exit(-1);
    }
